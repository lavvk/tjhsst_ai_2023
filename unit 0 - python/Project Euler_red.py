from time import perf_counter
start = perf_counter()

## skipped 18 & 24

def is_prime(x):
    if x < 2:
        return False
    if x == 2:
        return True
    if x % 2 == 0:
        return False
    for i in range(3, int(x**0.5) + 1, 2):
        if x % i == 0:
            return False
    return True

def factorial(n):
    result = 1
    for i in range(1, n + 1):
        result *= i
    return result

# 1
print("#1:", sum(x for x in range(1000) if x % 3 == 0 or x % 5 == 0))

#2
def fibonacci(limit):
    a, b = 1, 2
    total = 0
    while a <= limit:
        if a % 2 == 0:
            total += a
        a, b = b, a + b
    return total

print("#2:", fibonacci(4000000))

#3
def largest_prime_factor(n):
    i = 2
    while i * i <= n:
        if n % i:
            i += 1
        else:
            n //= i
    return n

print("#3:", largest_prime_factor(600851475143))

#4
print("#4:", max(x * y for x in range(100, 1000) for y in range(100, 1000) if str(x * y) == str(x * y)[::-1]))

#5
def gcd(x, y):
    while y != 0:
        temp = x
        x = y
        y = temp % y

    return x

def smallest_multiple(n):
    result = 1
    for i in range(1, n + 1):
        result = (result * i) // gcd(result, i)
    return result

print("#5:", smallest_multiple(20))

# 6
print("#6: ", sum(range(101))**2 - sum(x**2 for x in range(101)))


#7
count = 0
num = 1
while count < 10001:
    num += 1
    if is_prime(num):
        count += 1

print("#7:", num)

#8
number_str = "731671765313306249192251196744265747423553491949349698352031277450632623957831801698480186947885184385" +\
"861560789112949495459501737958331952853208805511125406987471585238630507156932909632952274430435576689" +\
"664895044524452316173185640309871112172238311362229893423380308135336276614282806444486645238749303589" +\
"072962904915604407723907138105158593079608667017242712188399879790879227492190169972088809377665727333" +\
"001053367881220235421809751254540594752243525849077116705560136048395864467063244157221553975369781797" +\
"784617406495514929086256932197846862248283972241375657056057490261407972968652414535100474821663704844" +\
"031998900088952434506585412275886668811642717147992444292823086346567481391912316282458617866458359124" +\
"566529476545682848912883142607690042242190226710556263211111093705442175069416589604080719840385096245" +\
"544436298123098787992724428490918884580156166097919133875499200524063689912560717606058861164671094050" +\
"7754100225698315520005593572972571636269561882670428252483600823257530420752963450"


number_str = number_str.replace("\n", "")
largest_product = 0

for i in range(len(number_str) - 12):
    product = 1
    for j in range(13):
        product *= int(number_str[i + j])
    largest_product = max(largest_product, product)

print("#8:", largest_product)

#9
def special_pythagorean_triplet(target_sum):
    for a in range(1, target_sum):
        for b in range(a, target_sum):
            c = target_sum - a - b
            if a**2 + b**2 == c**2:
                return a * b * c

print("#9: ", special_pythagorean_triplet(1000))


#11 -- kinda ugly python bc checking for diagonals not sure another way rip
grid_str = [[ 8,  2, 22, 97, 38, 15,  0, 40,  0, 75,  4,  5,  7, 78, 52, 12, 50, 77, 91,  8],

       [49, 49, 99, 40, 17, 81, 18, 57, 60, 87, 17, 40, 98, 43, 69, 48,  4, 56, 62,  0],

       [81, 49, 31, 73, 55, 79, 14, 29, 93, 71, 40, 67, 53, 88, 30,  3, 49, 13, 36, 65],

       [52, 70, 95, 23,  4, 60, 11, 42, 69, 24, 68, 56,  1, 32, 56, 71, 37,  2, 36, 91],

       [22, 31, 16, 71, 51, 67, 63, 89, 41, 92, 36, 54, 22, 40, 40, 28, 66, 33, 13, 80],

       [24, 47, 32, 60, 99,  3, 45,  2, 44, 75, 33, 53, 78, 36, 84, 20, 35, 17, 12, 50],

       [32, 98, 81, 28, 64, 23, 67, 10, 26, 38, 40, 67, 59, 54, 70, 66, 18, 38, 64, 70],

       [67, 26, 20, 68,  2, 62, 12, 20, 95, 63, 94, 39, 63,  8, 40, 91, 66, 49, 94, 21],

       [24, 55, 58,  5, 66, 73, 99, 26, 97, 17, 78, 78, 96, 83, 14, 88, 34, 89, 63, 72],

       [21, 36, 23,  9, 75,  0, 76, 44, 20, 45, 35, 14,  0, 61, 33, 97, 34, 31, 33, 95],

       [78, 17, 53, 28, 22, 75, 31, 67, 15, 94,  3, 80,  4, 62, 16, 14,  9, 53, 56, 92],

       [16, 39,  5, 42, 96, 35, 31, 47, 55, 58, 88, 24,  0, 17, 54, 24, 36, 29, 85, 57],

       [86, 56,  0, 48, 35, 71, 89,  7,  5, 44, 44, 37, 44, 60, 21, 58, 51, 54, 17, 58],

       [19, 80, 81, 68,  5, 94, 47, 69, 28, 73, 92, 13, 86, 52, 17, 77,  4, 89, 55, 40],

       [ 4, 52,  8, 83, 97, 35, 99, 16,  7, 97, 57, 32, 16, 26, 26, 79, 33, 27, 98, 66],

       [88, 36, 68, 87, 57, 62, 20, 72,  3, 46, 33, 67, 46, 55, 12, 32, 63, 93, 53, 69],

       [ 4, 42, 16, 73, 38, 25, 39, 11, 24, 94, 72, 18,  8, 46, 29, 32, 40, 62, 76, 36],

       [20, 69, 36, 41, 72, 30, 23, 88, 34, 62, 99, 69, 82, 67, 59, 85, 74,  4, 36, 16],

       [20, 73, 35, 29, 78, 31, 90,  1, 74, 31, 49, 71, 48, 86, 81, 16, 23, 57,  5, 54],

       [ 1, 70, 54, 71, 83, 51, 54, 69, 16, 92, 33, 48, 61, 43, 52,  1, 89, 19, 67, 48]]

def max_grid_product(grid):
    max_product = 0
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            product = 1
            if j + 3 < len(grid[i]):
                product = max(product, grid[i][j] * grid[i][j + 1] * grid[i][j + 2] * grid[i][j + 3])
            if i + 3 < len(grid):
                product = max(product, grid[i][j] * grid[i + 1][j] * grid[i + 2][j] * grid[i + 3][j])
            if i + 3 < len(grid) and j + 3 < len(grid[i]):
                product = max(product, grid[i][j] * grid[i + 1][j + 1] * grid[i + 2][j + 2] * grid[i + 3][j + 3])
            if i >= 3 and j + 3 < len(grid[i]):
                product = max(product, grid[i][j] * grid[i - 1][j + 1] * grid[i - 2][j + 2] * grid[i - 3][j + 3])
            max_product = max(max_product, product)
    return max_product

print("#11:", max_grid_product(grid_str))

# 12
def triangular_number(n):
    return n * (n + 1) // 2

def count_factors(num):
    count = 0
    for i in range(1, int(num**0.5) + 1):
        if num % i == 0:
            count += 2
    return count

def highly_divisible_triangle_number(divisor_count):
    n = 1
    while True:
        triangle = triangular_number(n)
        factors = count_factors(triangle)
        if factors > divisor_count:
            return triangle
        n += 1

print("#12: " , highly_divisible_triangle_number(500))


# 14
def collatz_sequence_length(n):
    count = 1
    while n != 1:
        if n % 2 == 0:
            n //= 2
        else:
            n = 3 * n + 1
        count += 1
    return count

max_length = 0
starting_number = 0

for i in range(1, 1000000):
    length = collatz_sequence_length(i)
    if length > max_length:
        max_length = length
        starting_number = i

print("#14:", starting_number)


#28
def spiral_diagonal_sum(n):
    if n == 1:
        return 1
    sum = 1
    current = 1
    for i in range(2, n + 1, 2):
        for x in range(4): 
            current += i
            sum += current
    return sum

print("#28:", spiral_diagonal_sum(1001))

#29
print("#29:", len(set(a**b for a in range(2, 101) for b in range(2, 101))))

#time calc
end = perf_counter()
print("Total time:", end - start)      

